...
public class GPUImage {  
     ...
     */
    public boolean supportsOpenGLES2(final Context context) {
    ...
    public void setUpCamera(final Camera camera) {
        setUpCamera(camera, 0, false, false, Rotation.NORMAL);
    }
    ...
    public void setUpCamera(final Camera camera, final int degrees, final boolean flipHorizontal,
            final boolean flipVertical, final Rotation defaultRotation) {
        mGlSurfaceView.setRenderMode(GLSurfaceView.RENDERMODE_CONTINUOUSLY);
        if (Build.VERSION.SDK_INT > Build.VERSION_CODES.GINGERBREAD_MR1) {
        	mRenderer.setUpPreviewSize(camera);
            setUpCameraGingerbread();
        } else {
        	mRenderer.setUpPreviewSize(camera);
            camera.setPreviewCallback(mRenderer);
            camera.startPreview();
            mRenderer.setPreviewStarted();
        }
    ...
    public boolean isPreviewStarted() {
    	return mRenderer.isPreviewStarted();
    }

    @TargetApi(11)
    private void setUpCameraGingerbread() {
        mRenderer.setUpSurfaceTexture();
    }
     ...
     */
    public void saveBitmapWithFilterApplied(String folderName, String fileName) {
        //return 
    	saveBitmapWithFilterApplied(mCurrentBitmap, folderName, fileName);
    }
     ...
     */
    public void saveBitmapWithFilterApplied(final Bitmap bitmap, String folderName, String fileName) {
        if (mGlSurfaceView != null) {
            mRenderer.deleteImage();
        ...
        buffer.saveBitmap(folderName, fileName);
    ...
    public static void getBitmapForMultipleFilters(final Bitmap bitmap,
            final List<GPUImageFilter> filters, final ResponseListener<Void> listener, String folderName, String fileName) {
        if (filters.isEmpty()) {
            return;
        }
        ...
        for (GPUImageFilter filter : filters) {
            ...
            listener.response(buffer.saveBitmap(folderName, fileName));
            filter.destroy();
        }
        renderer.deleteImage();
        buffer.destroy();
    ...
    private class SaveTask extends AsyncTask<Void, Void, Void> {
        ...
        public SaveTask(final Bitmap bitmap, final String folderName, final String fileName,
            ...
            Log.d("ASDF", "Savetask constructor1");
        }

        @Override
        protected Void doInBackground(final Void... params) {
        	Log.d("ASDF", "do in background1");
            //Bitmap result = 
//        	try {
//				Thread.sleep(1000);
//			} catch (InterruptedException e) {
...
//			}
        	try {
            	saveBitmapWithFilterApplied(mBitmap, mFolderName, mFileName);
                //saveImage(mFolderName, mFileName, result);
                final File file = new File(mFolderName + "/" + mFileName);
                ...
                if (mListener != null) {
                    mHandler.post(new Runnable() {
                        ...
                        @Override
                        public void run() {
                            mListener.onPictureSaved(file.toString());
                        }
                    });
                }
                ...
                if (mListener != null) {
                    mHandler.post(new Runnable() {
                        ...
                        @Override
                        public void run() {
                            mListener.onPictureSaveFailed(thr);
                        }
                    });
                }
        	}
...
/*
        private void saveImage(final String folderName, final String fileName, final Bitmap image) {
            ...
            final File file = new File(path, folderName + "/" + fileName);
            try {
                file.getParentFile().mkdirs();
                ...
                //AllocationMemory.savePhotoOpenCV(folderName + "/" + fileName, false, 0, byteArray, image.getWidth(), image.getHeight(), false);
    ...
    public interface OnPictureSavedListener {
        ...
        void onPictureSaveFailed(Throwable e);
    }
