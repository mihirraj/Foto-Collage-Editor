...
import com.wisesharksoftware.billing.Security.VerifiedPurchase;
...
public class BillingService extends Service implements ServiceConnection {
    ...
    public BillingService(String bString) {
        super();
	...
	public void handleCommand(Intent intent, int startId) {
    	if (intent == null) {
    		if (Consts.DEBUG) {
                Log.w(TAG, "handleCommand() intent is NULL");
            }
    		return;
    	}
    ...
    private void purchaseStateChanged(int startId, String signedData, String signature) {
        ...
        purchases = Security.verifyPurchase(getBaseContext(), signedData, signature, bString);
        if (purchases == null) {
            return;
        }
        ...
        // if doesn't purchased yet
        if (notifyList.isEmpty())
        {
        	ResponseHandler.purchaseResponse(this, PurchaseState.NONE, null,
                    null, 0, null);
        }
        if (!notifyList.isEmpty()) {
