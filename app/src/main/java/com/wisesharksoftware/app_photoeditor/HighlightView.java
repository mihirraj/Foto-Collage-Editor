...
public class HighlightView {
	...
	private Paint mAnimalEyesPaint = new Paint();
	...
	private boolean showAnimalEyes = false;
	private boolean dragByEdge = true;

	public boolean isDragByEdge() {
		return dragByEdge;
	}

	public void setDragByEdge(boolean dragByEdge) {
		this.dragByEdge = dragByEdge;
	}

	public boolean isShowAnimalEyes() {
		return showAnimalEyes;
	}

	public void setShowAnimalEyes(boolean showAnimalEyes) {
		this.showAnimalEyes = showAnimalEyes;
	}

	public HighlightView(View context, int styleId) {

		if (styleId > 0) {
			TypedArray appearance = context.getContext()
					.obtainStyledAttributes(styleId,
							R.styleable.AviaryCropHighlightView);

			mStrokeWidth = appearance.getDimensionPixelSize(
					R.styleable.AviaryCropHighlightView_aviary_strokeWidth, 2);
			mStrokeColor = appearance.getColor(
					R.styleable.AviaryCropHighlightView_aviary_strokeColor,
					Color.WHITE);
			mStrokeColorPressed = appearance.getColor(
					R.styleable.AviaryCropHighlightView_aviary_strokeColor2,
					Color.WHITE);

			mOutsideFillColor = appearance.getColor(
					R.styleable.AviaryCropHighlightView_aviary_color1,
					0x99000000);
			mOutsideFillColorPressed = appearance.getColor(
					R.styleable.AviaryCropHighlightView_aviary_color2,
					0x99000000);

			mStrokeWidth2 = appearance.getDimensionPixelSize(
					R.styleable.AviaryCropHighlightView_aviary_strokeWidth2, 1);
			mInternalStrokeColor = appearance.getColor(
					R.styleable.AviaryCropHighlightView_aviary_strokeColor3,
					Color.WHITE);
			mInternalStrokeColorPressed = appearance.getColor(
					R.styleable.AviaryCropHighlightView_aviary_strokeColor4,
					Color.WHITE);

			mResizeDrawable = appearance
					.getDrawable(R.styleable.AviaryCropHighlightView_android_src);
	...
	public void draw(Canvas canvas) {
		...
		if (isShowAnimalEyes()) {
			float cy = (mDrawRect.top + mDrawRect.bottom) / 2;
			float r = mDrawRect.height() / 3;
			float cx = mDrawRect.left + mDrawRect.width() / 5;
			float cx2 = mDrawRect.right - mDrawRect.width() / 5;		
			canvas.drawCircle(cx, cy, r, mAnimalEyesPaint);
			canvas.drawCircle(cx2, cy, r, mAnimalEyesPaint);				
		} else {
			float colStep = (float) mDrawRect.height() / grid_cols;
			float rowStep = (float) mDrawRect.width() / grid_rows;
	
			for (int i = 1; i < grid_cols; i++) {
				mLinesPath.moveTo((int) mDrawRect.left,
						(int) (mDrawRect.top + colStep * i));
				mLinesPath.lineTo((int) mDrawRect.right,
						(int) (mDrawRect.top + colStep * i));
			}
	
			for (int i = 1; i < grid_rows; i++) {
				mLinesPath.moveTo((int) (mDrawRect.left + rowStep * i),
						(int) mDrawRect.top);
				mLinesPath.lineTo((int) (mDrawRect.left + rowStep * i),
		...
		if (true /* || mMode == Mode.Grow */) {
			...
			if (mResizeDrawable != null) {
				...
				mResizeDrawable.setBounds(left - dWidth, top - dHeight, left
						+ dWidth, top + dHeight);
				mResizeDrawable.draw(canvas);
				mResizeDrawable.setBounds(right - dWidth, top - dHeight, right
						+ dWidth, top + dHeight);
				mResizeDrawable.draw(canvas);
				mResizeDrawable.setBounds(left - dWidth, bottom - dHeight, left
						+ dWidth, bottom + dHeight);
				mResizeDrawable.draw(canvas);
				mResizeDrawable.setBounds(right - dWidth, bottom - dHeight,
						right + dWidth, bottom + dHeight);
				mResizeDrawable.draw(canvas);
			}
		}
	}
	...
	public void setMode(Mode mode) {
		if (mode != mMode) {
			mMode = mode;
			mOutlinePaint.setColor(mMode == Mode.None ? mStrokeColor
					: mStrokeColorPressed);
			mOutlinePaint2.setColor(mMode == Mode.None ? mInternalStrokeColor
					: mInternalStrokeColorPressed);
			mLinesPaintShadow.setAlpha(mMode == Mode.None ? 102 : 0);
			mOutlineFill.setColor(mMode == Mode.None ? mOutsideFillColor
					: mOutsideFillColorPressed);
		}
	}
	...
	public int getHit(float x, float y) {
		...
		if (!dragByEdge) {
			if ((!(isLeftEdge(retval) && (isTopEdge(retval)))) &&  
				(!(isLeftEdge(retval) && (isBottomEdge(retval)))) &&
				(!(isRightEdge(retval) && (isTopEdge(retval)))) &&
				(!(isRightEdge(retval) && (isBottomEdge(retval))))
			)
			{
				retval = GROW_NONE;
			}
	...
	public void handleMotion(int edge, float dx, float dy) {
		...
		if (edge == GROW_NONE) {
			...
			moveBy(dx * (mCropRect.width() / tmpRect4.width()),
					dy * (mCropRect.height() / tmpRect4.height()));
		} else {
			...
			double xDelta = Math.round(dx
					* (mCropRect.width() / tmpRect4.width()));
			double yDelta = Math.round(dy
					* (mCropRect.height() / tmpRect4.height()));
	...
	void growWithConstantAspectSize(int edge, double dx, double dy) {
		...
		if (tmpRectMotionD.width() >= widthCap
				&& tmpRectMotionD.height() >= widthCap
				&& mImageRect.contains(tmpRectMotionD)) {
	...
	private RectD adjustRealCropRect(Matrix matrix, RectD rect,
			RectD outsideRect) {

		boolean adjusted = false;

		tempLayoutRectF.set((float) rect.left, (float) rect.top,
		...
		if (tempLayoutRectF.top < outsideRect.top) {
			...
			adjusted = true;
			rect.offset(0, -(tempLayoutRectF.bottom - outsideRect.bottom)
					/ scale);
		}
		...
		if (tempLayoutRectF.width() > outsideRect.width()) {
			...
			if (tempLayoutRectF.right > outsideRect.right)
				rect.right += -(tempLayoutRectF.right - outsideRect.right)
						/ scale;
		}
		...
		if (tempLayoutRectF.height() > outsideRect.height()) {
			...
			if (tempLayoutRectF.bottom > outsideRect.bottom)
				rect.bottom += -(tempLayoutRectF.bottom - outsideRect.bottom)
						/ scale;
		}
	...
	public void animateTo(final View parent, Matrix m, RectD imageRect,
			RectD cropRect, final boolean maintainAspectRatio) {
		...
		if (!mRunning) {
			...
			double ratio = (double) mCropRect.width()
					/ (double) mCropRect.height();
			...
			mHandler.post(new Runnable() {
				...
				public void run() {
					...
					double currentMs = Math.min(animationDurationMs, now
							- startTime);

					double value1 = mEasing.easeOut(currentMs, 0, pt1,
							animationDurationMs);
					double value2 = mEasing.easeOut(currentMs, 0, pt2,
							animationDurationMs);
					double value3 = mEasing.easeOut(currentMs, 0, pt3,
							animationDurationMs);
					double value4 = mEasing.easeOut(currentMs, 0, pt4,
							animationDurationMs);
	...
	public void setup(Matrix m, RectD imageRect, RectD cropRect,
		...
		try {
			ReflectionUtils.invokeMethod(mOutlinePaint, "setHinting",
		...
		try {
			ReflectionUtils.invokeMethod(mOutlinePaint2, "setHinting",
					new Class<?>[] { int.class }, 0);
		} catch (ReflectionException e) {
		}
		...
		mAnimalEyesPaint.setStrokeWidth(2);
		mAnimalEyesPaint.setAntiAlias(true);
		mAnimalEyesPaint.setColor(Color.WHITE);
		mAnimalEyesPaint.setStyle(Paint.Style.STROKE);
		mAnimalEyesPaint.setMaskFilter(new BlurMaskFilter(2, Blur.NORMAL));
