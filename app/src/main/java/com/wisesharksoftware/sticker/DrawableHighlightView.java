...
public class DrawableHighlightView implements OnSizeChange {
	...
	private int color = 0, alpha = 255;

	public void setMoveEnabled(boolean mMoveEnabled) {
		this.mMoveEnabled = mMoveEnabled;
	}

	public void setRotateEnabled(boolean mRotateEnabled) {
		this.mRotateEnabled = mRotateEnabled;
	}

	public DrawableHighlightView(ImageViewTouch context, int styleId,
			FeatherDrawable content) {
		mContent = content;
		mContext = context;
		...
		if (styleId > 0) {
			TypedArray array = context.getContext().obtainStyledAttributes(
					styleId, R.styleable.AviaryDrawableHighlightView);

			mAnchorRotate = array
					.getDrawable(R.styleable.AviaryDrawableHighlightView_aviary_rotateDrawable);
			mAnchorDelete = array
					.getDrawable(R.styleable.AviaryDrawableHighlightView_aviary_deleteDrawable);
			mBackgroundDrawable = array
					.getDrawable(R.styleable.AviaryDrawableHighlightView_android_background);
			mPadding = array
					.getDimensionPixelSize(
							R.styleable.AviaryDrawableHighlightView_android_padding,
							10);
			mResizeEdgeMode = array
					.getInteger(
							R.styleable.AviaryDrawableHighlightView_aviary_resizeEdgeMode,
							0);

			Log.i(LOG_TAG, "Edge Mode: " + mResizeEdgeMode);

			mMoveEnabled = array.getBoolean(
					R.styleable.AviaryDrawableHighlightView_aviary_moveEnabled,
					true);
			mRotateEnabled = array
					.getBoolean(
							R.styleable.AviaryDrawableHighlightView_aviary_rotateEnabled,
							true);
			mScaleEnabled = array
					.getBoolean(
							R.styleable.AviaryDrawableHighlightView_aviary_resizeEnabled,
							true);

			minSize = array
					.getDimension(
							R.styleable.AviaryDrawableHighlightView_aviary_minSize,
							20f);
	...
	public void setCropRect(RectF mCropRect) {
	...
	public void setColor(int color) {
		if (mContent != null && color != 0)
			mContent.setColorFilter(color, Mode.SRC_ATOP);
		this.color = color;
	}
	...
	protected RectF computeLayout() {
		if (mMoveEnabled && mRotateEnabled && mScaleEnabled) {
	...
	public void draw(final Canvas canvas) {
		...
		if (null != mBackgroundDrawable) {
			mBackgroundDrawable.setBounds((int) mTempRect.left,
					(int) mTempRect.top, (int) mTempRect.right,
		...
		if (mEditableContent != null) {
			mEditableContent.setBounds(mDrawRect.left, mDrawRect.top,
			...
			if (mShowAnchors) {
				...
				if (mAnchorRotate != null) {
					mAnchorRotate.setBounds(right - mAnchorRotateWidth, bottom
							- mAnchorRotateHeight, right + mAnchorRotateWidth,
							bottom + mAnchorRotateHeight);
					mAnchorRotate.draw(canvas);
				}

				if (mAnchorDelete != null) {
					mAnchorDelete.setBounds(left - mAnchorDeleteWidth, top
							- mAnchorDeleteHeight, left + mAnchorDeleteWidth,
	...
	public int getHit(float x, float y) {
		...
		if ((mRotateEnabled || mScaleEnabled)
				&& (Math.abs(rect.right - x) < HIT_TOLERANCE)
				&& (Math.abs(rect.bottom - y) < HIT_TOLERANCE) && verticalCheck
				&& horizCheck) {
			retval = ROTATE;
		}
	...
	public void onSingleTapConfirmed(float x, float y) {
		...
		if (mAnchorDelete != null) {
			if ((Math.abs(rect.left - x) < HIT_TOLERANCE)
					&& (Math.abs(rect.top - y) < HIT_TOLERANCE)
	...
	public Rect getInvalidationRect() {
		...
		mInvalidateRect.set((int) mInvalidateRectF.left,
				(int) mInvalidateRectF.top, (int) mInvalidateRectF.right,
	...
	public void setScaleEnabled(boolean mScaleEnabled) {
	...
	public void onMouseMove(int edge, MotionEvent event2, float dx, float dy) {
		...
		if (edge == MOVE) {
			...
			Log.d(LOG_TAG, "x: " + xDelta + ", y: " + yDelta + ", final: "
					+ delta);
	...
	public void invalidate() {
		...
		if (mDrawRect == null) {
			mDrawRect = getDisplayRect(mRotateMatrix, mCropRect);
		}
		mRotateMatrix.reset();
	...
	public void setup(final Context context, final Matrix m,
			final Rect imageRect, final RectF cropRect,
	...
	@Override
	public void onSizeChanged(EditableDrawable content, float left, float top,
		...
		if (content.equals(mEditableContent) && null != mContext) {
			...
			/*
			 * final Matrix mImageMatrix = mContext.getImageViewMatrix(); final
			 * Matrix matrix = new Matrix( mImageMatrix ); matrix.invert( matrix
			 * ); final float[] pts = new float[] { left, top, right, bottom };
			 * MatrixUtils.mapPoints( matrix, pts ); final RectF cropRect = new
			 * RectF( pts[0], pts[1], pts[2], pts[3] ); mCropRect.set( cropRect
			 * );
			 */
			...
			if (mDrawRect.left != left || mDrawRect.top != top
					|| mDrawRect.right != right || mDrawRect.bottom != bottom) {
				if (forceUpdate()) {
					mContext.invalidate(getInvalidationRect());
